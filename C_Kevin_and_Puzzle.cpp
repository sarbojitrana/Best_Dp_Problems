...
void solve(){
    ...
    vll v(n, 0);
    forn(i, n) cin >> v[i];
    vector<map<ll,ll>> truthdp(n), liedp(n);
    truthdp[0][v[0]] = v[0] == 0 ? 1 : 0;
    liedp[0][1] = 1;
    for(int i = 1; i< n; i++){
        truthdp[i][v[i]] = (truthdp[i-1][v[i]] + liedp[i-1][v[i]])%mod ;
        liedp[i][v[i-1] + 1] = truthdp[i-1][v[i-1]] ;
    }
    ll ans = truthdp.back()[v.back()];
    for(auto it : liedp.back()) ans = (ans + it.second)%mod ;
    cout << ans << endl;
}
