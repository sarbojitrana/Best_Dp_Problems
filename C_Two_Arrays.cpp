...
void solve() {
    ...
    vvll prev(n + 2, vll(n + 2, 0ll));

    for (ll ai = 1; ai <= n; ai++) {
        for (ll bi = ai; bi <= n; bi++) {
            prev[ai][bi] = 1;
        }
    }

    for (ll i = 1; i < m; i++) {
        vvll prefix(n + 2, vll(n + 2, 0ll));
        for (ll ai = 1; ai <= n; ai++) {
            for (ll bi = n; bi >= 1; bi--) {
                prefix[ai][bi] = (prev[ai][bi] + prefix[ai - 1][bi] + prefix[ai][bi + 1] - prefix[ai - 1][bi + 1] + mod) % mod;
            }
        }

        vvll curr(n + 2, vll(n + 2, 0ll));
        for (ll ai = 1; ai <= n; ai++) {
            for (ll bi = ai; bi <= n; bi++) {
                curr[ai][bi] = prefix[ai][bi];
            }
        }
