...
struct node{
    bool covbhnd, covfr, bcovbhnd, bcovfr;
    node(bool a = 0, bool b = 0, bool c = 0, bool d= 0){
        covbhnd = a;
        covfr = b;
        bcovbhnd = c;
        bcovfr = d; 
...
void solve() {
    ...
    for(int i =1; i< n; i++){
        if( i - v[i] >= 0){
            if(i == v[i] || dp[i - v[i] -1].covbhnd || dp[i-v[i] -1].bcovbhnd){
                dp[i-v[i]].bcovfr = 1;
                dp[i].covbhnd = 1;
            }
        }
        if(i + v[i] < n){
            if( dp[i-1].bcovbhnd || dp[i-1].covbhnd){
                dp[i + v[i]].bcovbhnd = 1;
                dp[i].covfr =  1;
            }
        }
    }
    if((dp[n-1].bcovbhnd | dp[n-1].covbhnd) && (dp[0].covfr | dp[0].bcovfr)) cout << "YES" << endl;
    else cout << "NO" << endl;
}
