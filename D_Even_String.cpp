...
void factorials(ll mod, ll maxi) {
    ...
    for(ll i = 2; i <= maxi; i++) fact[i] = (fact[i - 1] * i) % mod;
}
...
void solve() {
    ...
    forn(i, 26) {
        vll curr(even + 1, 0);
        for(ll sum = 0; sum <= even; sum++) {
            curr[sum] = prev[sum];
            if(sum >= v[i] && v[i]) {
                curr[sum] = (curr[sum] + prev[sum - v[i]]) % mod;
            }
        }
        prev = curr;
    }
    ...
    ll num = (fact[even] * fact[total - even]) % mod;
    ll ans = prev[even];
    ans = (ans * num) % mod;
    ans = (ans * binexp(deno, mod - 2, mod)) % mod;
